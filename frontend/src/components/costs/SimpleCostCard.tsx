...
interface SimpleCostCardProps {
  costData: CostData;
}

export function SimpleCostCard({ costData }: SimpleCostCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  if (!costData || (!costData.cost_eur && !costData.cost_usd)) {
    return null;
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600">
              Type: {costData.search_type === 'advanced' ? 'Recherche Avancée' : 'Recherche Simple'}
            </p>
          </div>
        </div>
        <button
          ...
          <div className="mt-4 p-3 bg-blue-100 rounded-lg text-xs text-blue-800">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                Taux de change USD→EUR: {costData.exchange_rate}
              </div>
            </div>
          </div>
        </div>
